...
combinedResidueCSV=sys.argv[2]
parentDirectory=sys.argv[3]
receptorName=sys.argv[4]

df=pd.read_csv(combineCSV)
fragmentResiduePairDF=pd.read_csv(combinedResidueCSV)
...
#print(meanOfEntire)
targets= fragmentResiduePairDF['fragment'].to_list()
theDict={}
entireMedian=df10.median()[0]

for target in targets:
    info={}
    theIndex=fragmentResiduePairDF.loc[fragmentResiduePairDF['fragment'] == target].index
    ...
    info['domain']=fragmentResiduePairDF.loc[theIndex[0], 'domain'] #added
...
dataDF.rename(columns={'index':'Fragment'},inplace=True) #changed Fragment to Key
#dataDF.drop('Fragment',axis=1,inplace=True)

os.chdir(parentDirectory)
dataDF.to_csv(receptorName + 'CombinedSpecificToResiduesResults.csv',index=False)
