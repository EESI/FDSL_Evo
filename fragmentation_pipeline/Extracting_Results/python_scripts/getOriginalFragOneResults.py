...
combinedOrigFragOneCSV=sys.argv[2]
parentDirectory=sys.argv[3]
receptorName=sys.argv[4]

df=pd.read_csv(combineCSV)
fragmentResiduePairDF=pd.read_csv(combinedOrigFragOneCSV)
...
#print(meanOfEntire)
targets= fragmentResiduePairDF['frag 1'].to_list()
theDict={}
entireMedian=df10.median()[0]

for target in targets:
    info={}
    theIndex=fragmentResiduePairDF.loc[fragmentResiduePairDF['frag 1'] == target].index
    ...
    info['frag 2']=fragmentResiduePairDF.loc[theIndex[0], 'frag 2'] #added
...
dataDF.rename(columns={'index':'Frag 1'},inplace=True) #changed Fragment to Key
#dataDF.drop('Fragment',axis=1,inplace=True)

os.chdir(parentDirectory)
dataDF.to_csv(receptorName + 'CombinedGenericToSpecificResults.csv',index=False)
