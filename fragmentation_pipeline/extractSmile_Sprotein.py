...
def getAllInfoIntoDict(xmlDir,ligandDir):
    ...
    tmpString[2] is used more commonly, but for modified, its tmpString[0]
    '''
    masterdict = {}
    iterator=0
    ...
    for filename in os.listdir(xmlDir):
        details = {}
        tmpString = filename.split('.')
        print("This is the tmString: ",tmpString)
        print(tmpString[0])
        errors=[]
        try:
            dictsOfSmileToPDB = getDictGenerated(parser.getSMILE_PDBCONVERSION(os.path.join(xmlDir,filename)))
            result =  parser.parse_ligand_indices(os.path.join(xmlDir,filename))
            interactingAtoms = result[0]      
            LigToProDict = result[1]
            residueDict=result[2]
            SMILE = parser.getSMILE(os.path.join(xmlDir,filename))
            ...
            print("ERROR: ",e, "Hence, value skipped for filename: ",filename)
            continue 
        #details['FragToDomDict']=fragToDomDict
        masterdict[tmpString[0]] = details
        ...
        for fragment in masterdict[ligand]['interactingLigands']:
            ...
            for atom in proteinSet:
                try:
                    ...
                    print("This atom cannot be added: ",atom)
