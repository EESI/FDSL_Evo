...
class PDBQTprep:
    ...
    def files_preparation(self):
        def prepare_ligand():
            ...
            return flex_df.to_pdb(path=os.getenv('TMPDIR') + '/pdbForPLIP/{}-prep.pdb'.format(self.flexname.split('.')[0]),
                                  records=['ATOM', 'HETATM'],
                                  gz=False,
                                  append_newline=True)
        ...
        def prepare_protein():
            ...
            return prot_df.to_pdb(path=os.getenv('TMPDIR') + '/pdbForPLIP/{}-dehyd.pdb'.format(self.protname.split('.')[0]),
                                  records=['ATOM', 'HETATM'],
                                  gz=False,
                                  append_newline=True)
        ...
        prepare_ligand()
        print(self.plip)
        return Converter(os.getenv('TMPDIR') + '/pdbForPLIP/{}-dehyd.pdb'.format(self.protname.split('.')[0]),
                         os.getenv('TMPDIR') + '/pdbForPLIP/{}-prep.pdb'.format(self.flexname.split('.')[0]), self.plip).convert()
